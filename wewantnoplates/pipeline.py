"""End-to-end pipeline: input image + prompt -> transformed image.
   ...
   4. Save the image (plus a sidecar JSON recording the prompt) to the output
      directory.
"""
...
def run_pipeline(
    ...
    *,
    ...
    # 4. Generate (local image-to-image) and save ------------------------------
    output_path = None
    if not dry_run:
        gen = generator if generator is not None else build_generator()
        result = gen.generate(
            image=src,
            prompt=understanding.transformation_prompt,
            width=target[0],
            height=target[1],
            seed=seed if seed is not None else config.GENERATOR_SEED,
        )
